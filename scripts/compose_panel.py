...
def cover(img: Image.Image, size: tuple[int, int], anchor: str, resample=Image.LANCZOS) -> Image.Image:
...
def main(argv: list[str] | None = None) -> int:
    ...
    adaptive_design = (
        ...
        else design_src.resize((pw, ph), Image.Resampling.NEAREST)
    )

    if args.layout == "design-only":
        print(f"layout design-only, canvas {cw}x{ch}, no seam")
        print(overflow_report(design_src, (cw, ch), "design"))
        result = adaptive_design if adaptive else cover(design_src, (cw, ch), "center", Image.Resampling.NEAREST)
    else:
        ...
        design_panel = adaptive_design if adaptive else cover(design_src, (pw, ph), "center", Image.Resampling.NEAREST)
